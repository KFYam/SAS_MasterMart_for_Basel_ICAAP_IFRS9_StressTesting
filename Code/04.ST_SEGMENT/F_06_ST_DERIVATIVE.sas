data MART.STICAAP_06_DERIVATIVE_&st_Rptmth.;
	set STG.STICAAP_06_DERIVATIVE_&st_Rptmth.;

	length	RWA_ST0		- RWA_ST3	8;
	length	RW_ST0		- RW_ST3	8;
	length	CRM_ST0		- CRM_ST3	8;
	
	array a_npldta		{4}	_temporary_ (&npl_deriv0.	&npl_deriv1.	&npl_deriv2. 	&npl_deriv3.);
	array a_curr_mult	{4}	_temporary_ (&dcurr_m0. 	&dcurr_m1.		&dcurr_m2. 		&dcurr_m3.);
	...
	*/
	EAD			= APPL_CRM_AMT_HKE;/*POTENT_EXP_AMT_HKE+CUR_EXP_AMT_HKE;*/
...
run;	
/* Note:
Actually, not pastdue in Derivative real case, it counterparty (borrower) early call off, the Bank bare loss only;
Using NPL concept of Derivative because most corporate is unrated and so that the RW = 100 specifically on 
portfolios of "Exchange rate and gold contracts to corporate counterparties" and "Interest rate contracts to 
corporate counterparties". Under stress condition, the MTM frustrates, the Bank can gain or loss, only RW can been 
stressed but IA for Derivative is not necessary 
*/
