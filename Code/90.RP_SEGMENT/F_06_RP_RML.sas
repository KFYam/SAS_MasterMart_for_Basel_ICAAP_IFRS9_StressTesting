...
proc sort data=STG.RP_01_HK_RML_&st_Rptmth. out=RP_hkrml;		by ACCT_ID; run;
proc sort data=siw.vi_cmv_aclevel_&st_RptMth. out=cmv_aclevel;	by ACCT_ID; run;

data MART.RP_01_HK_RML_&st_Rptmth.;
	merge RP_hkrml(in=a) cmv_aclevel(in=b);
	by acct_id; 
	if a;
	...
	array b_cov_ratio	RP_COV_RATIO0	- RP_COV_RATIO3;
	array b_crm_npl_s	RP_crm_npl_s0	- RP_crm_npl_s3;
	array b_crm_npl_c	RP_crm_npl_c0	- RP_crm_npl_c3;
